...
public class Utility {
    private static final String SCREENSHOTS_PATH = "test-outputs/target/Screenshots/";
    ...
    public List<WebElement> ListOFElement(WebDriver driver, By locator) {
        new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.elementToBeClickable(locator));
    ...
    public static void  hoverElement(WebDriver driver, WebElement hoverElement){
        Actions actions = new Actions(driver);
        actions.moveToElement(hoverElement).pause(500).perform();
    }

    public static void hoverTheElements(WebDriver driver, List<WebElement> title, int index) {
        if (title.size() > index-1) {
            hoverElement( driver,title.get(index-1));
        }
    }

    public static void clickOnListElementsIndex
            (List<WebElement> title, int index) {
        if (title.size() > index-1) {
            title.get(index-1).click();
        } else {
            System.out.println("Not enough elements found");
        }
    }

    public static String GetProductNameInCartById
            (List<WebElement> title, int index) {
        if (title.size() > index-1) {
            title.get(index-1).getText();
        } else {
            System.out.println("Not enough elements found");
        }
        return title.get(index-1).getText();
    }
